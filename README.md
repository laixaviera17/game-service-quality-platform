...
| 服务器性能与数据质量 | 事务扣减库存、质量规则、按严重级别输出报告 |
| AI 自动化加分 | 规则检查结果可作为后续 LLM 异常归因 / 用例生成的结构化输入 |
...
- 基于 Python、FastAPI 与 SQLite 搭建活动奖励发放服务，设计幂等键、事务扣减与库存校验，覆盖重复请求、库存不足等线上高风险场景。
- 编写自动化测试与数据质量规则，检查重复发奖、孤儿记录、非法状态和负余额，并输出结构化质量报告与可视化页面。
- 将数据质量检测、接口测试与异常定位沉淀为可复用工具链；后续可接入 LLM 进行失败用例归因和测试建议生成。

不要写“已接入 LLM”或“支撑真实游戏业务”，除非你确实完成并能演示。
